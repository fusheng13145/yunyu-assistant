...
- **接口级冒烟（v2.30 起，v2.31 首次对真实实例执行）**：`scripts/smoke.sh` 对**已运行的实例**打一遍 HTTP 与 WS 链路（9 节 62 项：actuator 暴露面与端口分离、未匹配路径返回 404 而非 500、401/403 与伪造令牌、注册→me→refresh 轮换、助手/会话 CRUD 写后读、配额结构、WS 握手与站点 Origin 白名单、管理端只读、登出后黑名单）——单测证明方法行为，这里证明装配、拦截器、序列化与握手。用法 `BASE=... MGMT_BASE=... SMOKE_ORIGIN=... scripts/smoke.sh`，退出码 0/1/2，边界与不触碰的接口见脚本头部注释