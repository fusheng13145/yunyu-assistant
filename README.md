...
| 模块 | 能力 |
|---|---|
| 账号认证 | 注册 / 登录 / 修改密码 / 个人资料，JWT 无状态鉴权 + 双令牌**静默续期**（临期或 401 时单飞刷新后重放，跨标签页互不踢出，v2.30），登录限流防爆破 |
...
| `MANAGEMENT_SERVER_PORT` / `MANAGEMENT_SERVER_ADDRESS` | **（v2.30）** actuator 端口与监听地址，默认"与业务同端口、地址回落 `SERVER_ADDRESS`"。⚠️ Boot 的独立管理端口**不继承** `server.address`（只认 `management.server.address`，不配即绑全网卡），故公网实例换端口时必须两项一起配，否则 `/actuator` 反而更暴露（手册 6.1 与 7.4 C-59） |
...
- **接口级冒烟（v2.30）**：`scripts/smoke.sh` 对**已运行的实例**打一遍 HTTP 与 WS 链路（9 节 61 项：actuator 暴露面与端口分离、401/403 与伪造令牌、注册→me→refresh 轮换、助手/会话 CRUD 写后读、配额结构、WS 握手与站点 Origin 白名单、管理端只读、登出后黑名单）——单测证明方法行为，这里证明装配、拦截器、序列化与握手。用法 `BASE=... MGMT_BASE=... SMOKE_ORIGIN=... scripts/smoke.sh`，退出码 0/1/2，边界与不触碰的接口见脚本头部注释